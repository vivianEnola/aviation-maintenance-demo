...
def analyze_cloud(report: AnalysisReport, rules: dict[str, Any]) -> None:
    ...
    if "cumulonimbus" in labels:
        report.recommendations.append(
            "积雨云可能伴随雷暴、冰雹、积冰、强降水、风切变和强烈垂直气流；应依据适用的运行手册、气象资料和管制指令提早规划绕飞，并做好客舱安全与防颠簸准备。图像识别结果仅作辅助参考，请结合机载气象雷达和官方气象信息进一步确认。"
        )
    if "stratocumulus" in labels:
        report.recommendations.append(
            "层积云通常低于强对流风险，但仍需核查云底高、能见度、颠簸、降水、零度层和积冰条件。巡航或进近方案应依据适用的运行手册、气象资料和管制指令制定；如存在积冰条件，应按机型程序使用防冰系统，不应仅凭图像结果决定改变高度、实施目视进近或启用防冰。"
        )
